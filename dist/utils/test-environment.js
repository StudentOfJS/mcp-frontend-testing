...
export async function setupTestEnvironment(testDir, sourceCode, testCode, framework, type, config) {
    ...
    if (framework === 'jest') {
        await writeFile(path.join(testDir, 'jest.config.js'), config?.jestConfig || getDefaultJestConfig(type, isTypeScript));
        // Setup for React testing
        if (isReact) {
            await writeFile(path.join(testDir, 'setupTests.js'), `import '@testing-library/jest-dom';`);
        }
    }
    else if (framework === 'cypress') {
        await writeFile(path.join(testDir, 'cypress.config.js'), config?.cypressConfig || getDefaultCypressConfig(type, isTypeScript));
        // Create cypress directory structure for e2e tests
        if (type === 'e2e') {
            await mkdir(path.join(testDir, 'cypress', 'e2e'), { recursive: true });
            await writeFile(path.join(testDir, 'cypress', 'e2e', `${name}.cy.js`), testCode);
        }
        // Setup for component testing
        if (type === 'component') {
            await mkdir(path.join(testDir, 'cypress', 'support'), { recursive: true });
            await writeFile(path.join(testDir, 'cypress', 'support', 'component.js'), `import { mount } from 'cypress/react18'
import './commands'
Cypress.Commands.add('mount', mount)`);
            await writeFile(path.join(testDir, 'cypress', 'support', 'commands.js'), `// Custom commands go here`);
        }
    }
    // Set up package.json
    ...
    if (framework === 'cypress' && type === 'e2e') {
        await writeFile(path.join(testDir, 'index.html'), `<!DOCTYPE html>
<html>
<head>
...
<body>
  ...
  <script type="module" src="./index.js"></script>
        ...
        await writeFile(path.join(testDir, 'index.js'), `import React from 'react';
...
export async function setupComponentTestEnvironment(testDir, componentCode, testCode, framework, props) {
    ...
    if (props) {
        await writeFile(path.join(testDir, 'props.json'), JSON.stringify(props, null, 2));
        // Create a wrapper component for the tests
        const { wrapperCode, importStatement } = getComponentTestWrappers(name, isTypeScript);
        await writeFile(path.join(testDir, `TestWrapper${sourceExt}`), `${importStatement}
import ${name} from './${name}';
...
${wrapperCode}`);
    ...
    if (framework === 'jest') {
        await writeFile(path.join(testDir, 'jest.config.js'), getDefaultJestConfig('component', isTypeScript));
        await writeFile(path.join(testDir, 'setupTests.js'), `import '@testing-library/jest-dom';`);
    }
    else {
        await writeFile(path.join(testDir, 'cypress.config.js'), getDefaultCypressConfig('component', isTypeScript));
        await mkdir(path.join(testDir, 'cypress', 'support'), { recursive: true });
        await writeFile(path.join(testDir, 'cypress', 'support', 'component.js'), `import { mount } from 'cypress/react18'
import './commands'
Cypress.Commands.add('mount', mount)`);
...
export async function executeTest(testDir, framework, type) {
...
export async function executeComponentTest(testDir, framework) {
